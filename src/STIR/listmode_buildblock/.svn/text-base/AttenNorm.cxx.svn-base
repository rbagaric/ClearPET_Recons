...
initialise_keymap()
{
  parser.add_start_key("atten_norm Parameters");
...
post_processing()
{
  ...
  cerr << "AttenNorm Using FRAME_BASED_DT_CORR\n";
#else
  cerr << "AttenNorm NOT Using FRAME_BASED_DT_CORR\n";
#endif
  return false;
}
...
AttenNorm::
AttenNorm(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
...
process_data()
{
  ...
  double expFactor = -0.0096 * norm_data_info_ptr->get_scanner_ptr()->get_default_bin_size();
  for (int segment_index = norm_data_ptr->get_min_segment_num(); segment_index <= norm_data_ptr->get_max_segment_num(); segment_index++)
  {
    ...
    for (int axial_index = norm_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= norm_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = norm_data_ptr->get_min_view_num(); view_index <= norm_data_ptr->get_max_view_num(); view_index++)
      {
        ...
        {
		  (*segments_out[segment_index])[view_index][axial_index][tangential_index] = exp(expFactor*segNorm[view_index][axial_index][tangential_index]);
        } //end tangential
      } //end view
    }  //end axial
