...
initialise_keymap()
{
  parser.add_start_key("average_sino Parameters");
...
AverageSino::
AverageSino(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
    ...
    for (int axial_index = sino_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= sino_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      if (flag == 0)
      {
        ...
        for (int view_index = sino_data_ptr->get_min_view_num(); view_index <= sino_data_ptr->get_max_view_num(); view_index++)
        {
          ...
          {
              TotalSino += segSino[view_index][axial_index][tangential_index] ;
              NbSino += 1 ;
          } //end tangential
        } //end view
        ...
        for (int view_index = sino_data_ptr->get_min_view_num(); view_index <= sino_data_ptr->get_max_view_num(); view_index++)
        {
          ...
          {
            if (TotalSino != 0.0)
              (*segments_out[segment_index])[view_index][axial_index][tangential_index] = ( segSino[view_index][axial_index][tangential_index] * NbSino) / TotalSino;
            else
              (*segments_out[segment_index])[view_index][axial_index][tangential_index] = segSino[view_index][axial_index][tangential_index];
          } //end tangential
        } //end view
      } // end flag=0
