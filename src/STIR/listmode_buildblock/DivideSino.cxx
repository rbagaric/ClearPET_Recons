...
#include "stir/listmode/DivideSino.h"
...
initialise_keymap()
{
  parser.add_start_key("divide_sino Parameters");
...
DivideSino::
DivideSino(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
  ...
  for (int segment_index = sino1_data_ptr->get_min_segment_num(); segment_index <= sino1_data_ptr->get_max_segment_num(); segment_index++)
  {
    ...
    for (int axial_index = sino1_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= sino1_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = sino1_data_ptr->get_min_view_num(); view_index <= sino1_data_ptr->get_max_view_num(); view_index++)
      {
        ...
        {
		  if (segSino2[view_index][axial_index][tangential_index] != 0)
            (*segments_out[segment_index])[view_index][axial_index][tangential_index] = segSino1[view_index][axial_index][tangential_index] / segSino2[view_index][axial_index][tangential_index];
		  else
            (*segments_out[segment_index])[view_index][axial_index][tangential_index] = 0.0;
        } //end tangential
      } //end view
    }  //end axial
