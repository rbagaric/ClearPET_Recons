...
#include "stir/listmode/FillGap.h"
...
initialise_keymap()
{
  parser.add_start_key("fill_gap Parameters");
  parser.add_key("sino_init file",&sino_init_filename);
  parser.add_key("sino_fwd file",&sino_fwd_filename);
...
post_processing()
{
  sino_init_data_ptr = ProjData::read_from_file(sino_init_filename);
  sino_init_data_info_ptr = sino_init_data_ptr->get_proj_data_info_ptr()->clone();

  sino_fwd_data_ptr = ProjData::read_from_file(sino_fwd_filename);
  sino_fwd_data_info_ptr = sino_fwd_data_ptr->get_proj_data_info_ptr()->clone();

  num_segments = sino_init_data_ptr->get_num_segments();
  printf("Seg %d\n",num_segments);
  ...
  cerr << "FullNorm Using FRAME_BASED_DT_CORR\n";
#else
  cerr << "FullNorm NOT Using FRAME_BASED_DT_CORR\n";
#endif
  return false;
}
...
process_data()
{
  ...
  VectorWithOffset<segment_type *> segments_out (sino_init_data_info_ptr->get_min_segment_num(), sino_init_data_info_ptr->get_max_segment_num());
  ...
  proj_data_out_ptr = construct_proj_data(output, output_filename, sino_init_data_info_ptr);

  allocate_segments(segments_out, sino_init_data_ptr->get_min_segment_num(), sino_init_data_ptr->get_max_segment_num(), proj_data_out_ptr->get_proj_data_info_ptr());

  for (int segment_index = sino_init_data_ptr->get_min_segment_num(); segment_index <= sino_init_data_ptr->get_max_segment_num(); segment_index++)
  {
    printf("Segments %d\n",segment_index);
    cerr << "Processing next batch of segments" <<endl;

    SegmentByView<elem_type> segSinoInit = sino_init_data_ptr->get_segment_by_view(segment_index);
    SegmentByView<elem_type> segSinoFwd = sino_fwd_data_ptr->get_segment_by_view(segment_index);

    for (int axial_index = sino_init_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= sino_init_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = sino_init_data_ptr->get_min_view_num(); view_index <= sino_init_data_ptr->get_max_view_num(); view_index++)
      {
        for (int tangential_index = sino_init_data_ptr->get_min_tangential_pos_num(); tangential_index <= sino_init_data_ptr->get_max_tangential_pos_num(); tangential_index++)
        {
		  if (segSinoInit[view_index][axial_index][tangential_index] == 0)
		    (*segments_out[segment_index])[view_index][axial_index][tangential_index] = segSinoFwd[view_index][axial_index][tangential_index];
		  else
		    (*segments_out[segment_index])[view_index][axial_index][tangential_index] = segSinoInit[view_index][axial_index][tangential_index];
        } //end tangential
      } //end view
    }  //end axial
  } // end of for loop for segment range

  save_and_delete_segments(output, segments_out, sino_init_data_ptr->get_min_segment_num(), sino_init_data_ptr->get_max_segment_num(), *proj_data_out_ptr);
  proj_data_out_ptr = 0;
