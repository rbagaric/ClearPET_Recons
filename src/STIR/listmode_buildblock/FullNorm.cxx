...
#include "stir/listmode/FullNorm.h"
...
initialise_keymap()
{
  parser.add_start_key("full_norm Parameters");
...
FullNorm::
FullNorm(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
  ...
  for (int segment_index = norm_data_ptr->get_min_segment_num(); segment_index <= norm_data_ptr->get_max_segment_num(); segment_index++)
  {
    ...
    for (int axial_index = norm_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= norm_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = norm_data_ptr->get_min_view_num(); view_index <= norm_data_ptr->get_max_view_num(); view_index++)
      {
        ...
        {
            if (segNorm[view_index][axial_index][tangential_index] != 0.0)
            {
              (*segments_out[segment_index])[view_index][axial_index][tangential_index] = 1 / segNorm[view_index][axial_index][tangential_index];
              compteur_tang++;
            }
        } //end tangential
        ...
        for (int tangential_index = norm_data_ptr->get_min_tangential_pos_num(); tangential_index <= norm_data_ptr->get_max_tangential_pos_num(); tangential_index++)
        {
            if (segNorm[view_index][axial_index][tangential_index] != 0.0)
            {
              // (*segments_out[segment_index])[view_index][axial_index][tangential_index] = Max_normFC;

            }
        } //end tangential


        for (int tangential_index = norm_data_ptr->get_min_tangential_pos_num(); tangential_index <= norm_data_ptr->get_min_tangential_pos_num(); tangential_index++)
        {
            //(*segments_out[segment_index])[view_index][axial_index][tangential_index] = Max_normFC;
        } //end tangential

        for (int tangential_index = norm_data_ptr->get_max_tangential_pos_num(); tangential_index <= norm_data_ptr->get_max_tangential_pos_num(); tangential_index++)
        {
            //(*segments_out[segment_index])[view_index][axial_index][tangential_index] = Max_normFC;
        } //end tangential
        ...
        for (int tangential_index = norm_data_ptr->get_min_tangential_pos_num(); tangential_index <= norm_data_ptr->get_max_tangential_pos_num(); tangential_index++)
        {
            if (segNorm[view_index][axial_index][tangential_index] != 0.0)
            {
              ...
              //(*segments_out[segment_index])[view_index][axial_index][tangential_index] /= Moy;
            //(*segments_out[segment_index])[view_index][axial_index][tangential_index] *= (*segments_raw[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()];
            }
        } //end tangential
      } //end view
    }  //end axial
...
//Décommenter pour calculer et étendre l'inverse d'une norme par une view
/*    for (int axial_index = norm_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= norm_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int tangential_index = norm_data_ptr->get_min_tangential_pos_num(); tangential_index <= norm_data_ptr->get_max_tangential_pos_num(); tangential_index++)
      {
		...
		)
  	  {
            ...
            //{
              //Somme sur une view toutes les views
              (*segments_temp[bin_out.segment_num()])[norm_data_ptr->get_min_view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()] += (*segments_norm[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()];
    ...
    for (int axial_index = norm_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= norm_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = norm_data_ptr->get_min_view_num(); view_index <= norm_data_ptr->get_max_view_num(); view_index++)
      {
		...
		)
  	  {
            ...
            //if((*segments_norm[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()]!=0.0)
            //{
              (*segments_out[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()] = 1/(*segments_temp[bin_out.segment_num()])[norm_data_ptr->get_min_view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()];
