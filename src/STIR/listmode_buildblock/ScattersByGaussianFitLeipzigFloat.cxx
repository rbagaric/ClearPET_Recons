...
#include "stir/listmode/ScattersByGaussianFitLeipzig.h"
...
ScattersByGaussianFitLeipzig::
ScattersByGaussianFitLeipzig(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
...
printf("nb_tang_max_init %d\n",nb_tang_max_init);
    ...
    //double sfcorrFOV9[nb_tang_max][nb_plan_max][nb_view_max];
    //double objectFOV9[nb_tang_max][nb_plan_max][nb_view_max];

    float Factor=1.0;
    float SFobj=1.0;
    float ObjCrit=1.0;
    float reshapesfcorr=1.0;
    
    if( nb_tang_max_init > 100 ) nb_tang_max = nb_tang_max_init-20 ;
    else nb_tang_max = nb_tang_max_init ;
    ...
    if( nb_tang_max_init > 100 )
    {
      ...
      //double sino_EInit[nb_tang_max][nb_plan_max][nb_view_max];
      ...
      for (int axial_index = emis_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= emis_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
      {
		...
		&& bin_out.axial_pos_num() <= proj_data_out_ptr->get_max_axial_pos_num(bin_out.segment_num())
  	    ...
  	    {
              sino_E[tangential_index+(int)(nb_tang_max/2)][axial_index][view_index] = (*segments_emis[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()];
//printf("Tang %d View %d Axial %d sinoE %f sinoemis %f\n",tangential_index+(int)(nb_tang_max/2),view_index,axial_index,sino_E[tangential_index+(int)(nb_tang_max/2)][axial_index][view_index],(*segments_emis[bin_out.segment_num()])[bin_out.view_num()][bin_out.axial_pos_num()][bin_out.tangential_pos_num()]);
    ...
    {
      //double sino_E[nb_tang_max_init][nb_plan_max][nb_view_max];
      //double sino_EInit[nb_tang_max_init][nb_plan_max][nb_view_max];
...
//printf("Tang %d View %d Axial %d sinoE %f\n",T,V,A,sino_E[T][A][V]);
    ...
    float compteur_somme_total = 0.0 ;
...
// Recherche des sinos vides
    for ( int A = 0 ; A<nb_plan_max ; A++)
    {
      for ( int V = 0 ; V<nb_view_max ; V++)
      {
        ...
        {
          if ( sino_E[L][A][V] == 0.0 ) cont[V][A]=cont[V][A]+1;
        } //end L
      } // end V
    }  //end A
...
//printf("%f\n",compteur_somme_total);
    ...
    for ( int A = 0 ; A<nb_plan_max ; A++)
    {
      for ( int V = 0 ; V<nb_view_max ; V++)
      {
        OP[A][V] = (float)(compteur_somme_profil[A][V]/compteur_somme_total)*1000000. ;
        centerofmass[A][V] = (float)(sumcenterofmass[A][V]/compteur_somme_profil[A][V]);
...
//if ( segment_index==0 && A==30 && V==40 ) printf ("tang %d, E %f, E_liss %f\n",T,sino_E[T][A][V],sino_E_lissT[T][A][V]);
            ...
            if (compteur != 0) sino_E_lissT[T][A][V] /= (compteur);
...
// Lissage view l=8
        for ( int V = 0 ; V<nb_view_max ; V++)
        {
          for (int T = 0; T < nb_tang_max ; T++)
          {
            ...
            if (compteur != 0) sino_E[T][A][V] /= (compteur);
...
// Linear Fit
          ...
          float Xcoord1[nb_tang_max];
          float SFcorrected[nb_tang_max];
          float premder[nb_tang_max];
          float result[2];
          float maxpremder = 0.0 ;
          float minpremder = 1000000000.0 ;
          int J = 0 ;
          int K = 0 ;
          int LowLimit;
          int HighLimit;
...
//if ( segment_index==0 && A==30 && V==40 )printf ("OP %f,  centerofmass %f\n",OP[A][V],centerofmass[A][V]);
          ...
          if (b < 0.0 || b > nb_tang_max) b = nb_tang_max/2.0;
          if (c < 20.0) c = 20.0;
          if (c > 30.0) c = 30.0;


          float area = compteur_somme_profil[A][V]*SF/100.0 ;
          float a = area / ( -1.25331*c*erf( (-56.56854 + 0.70710*b)/c ) + 1.25331*c*erf( (0.70710*b)/c ) );
...
//if ( segment_index==0 && A==1 /*&& (i==40) && V==40*/ )printf ("P[0] %f\n",p[0]);
          ...
          for ( int i=0 ; i < nb_tang_max ; i++)
          {
            ...
            float Z = (Xcoord1[i] - centergauss) / p[2] ;
