...
initialise_keymap()
{
  parser.add_start_key("subtract_sino Parameters");
  parser.add_key("sino1 file",&sino1_filename);
  parser.add_key("sino2 file",&sino2_filename);
...
post_processing()
{
  sino1_data_ptr = ProjData::read_from_file(sino1_filename);
  sino1_data_info_ptr = sino1_data_ptr->get_proj_data_info_ptr()->clone();

  sino2_data_ptr = ProjData::read_from_file(sino2_filename);
  sino2_data_info_ptr = sino2_data_ptr->get_proj_data_info_ptr()->clone();

  num_segments = sino1_data_ptr->get_num_segments();
  printf("Seg %d\n",num_segments);
  ...
  cerr << "SubtractSino Using FRAME_BASED_DT_CORR\n";
#else
  cerr << "SubtractSino NOT Using FRAME_BASED_DT_CORR\n";
#endif
  return false;
}
...
SubtractSinoScatterNegA0::
SubtractSinoScatterNegA0(const char * const par_filename)
{
  set_defaults();
  if (par_filename!=0)
    {
...
process_data()
{
  ...
  VectorWithOffset<segment_type *> segments_out (sino1_data_info_ptr->get_min_segment_num(), sino1_data_info_ptr->get_max_segment_num());
  ...
  proj_data_out_ptr = construct_proj_data(output, output_filename, sino1_data_info_ptr);
  allocate_segments(segments_out, sino1_data_ptr->get_min_segment_num(), sino1_data_ptr->get_max_segment_num(), proj_data_out_ptr->get_proj_data_info_ptr());

  for (int segment_index = sino1_data_ptr->get_min_segment_num(); segment_index <= sino1_data_ptr->get_max_segment_num(); segment_index++)
  {
    double SF = 0.0 ;
    int compt_SF = 0 ;
    ...
    SegmentByView<elem_type> segSino1 = sino1_data_ptr->get_segment_by_view(segment_index);
    SegmentByView<elem_type> segSino2 = sino2_data_ptr->get_segment_by_view(segment_index);

    for (int axial_index = sino1_data_ptr->get_min_axial_pos_num(segment_index); axial_index <= sino1_data_ptr->get_max_axial_pos_num(segment_index); axial_index++)
    {
      for (int view_index = sino1_data_ptr->get_min_view_num(); view_index <= sino1_data_ptr->get_max_view_num(); view_index++)
      {
        double compt_scatter = 0.0;
        double compt_emis = 0.0;

        for (int tangential_index = sino1_data_ptr->get_min_tangential_pos_num(); tangential_index <= sino1_data_ptr->get_max_tangential_pos_num(); tangential_index++)
        {
          (*segments_out[segment_index])[view_index][axial_index][tangential_index] = std::max(float(0.0), segSino1[view_index][axial_index][tangential_index] - segSino2[view_index][axial_index][tangential_index]);

          if ( segment_index==0 && ((axial_index>=15 && axial_index<=19) || (axial_index>=28 && axial_index<=32)) )
          {
            compt_scatter += (*segments_out[segment_index])[view_index][axial_index][tangential_index];
            compt_emis += segSino1[view_index][axial_index][tangential_index];
          }
        } //end tangential

        if (segment_index==0 && ((axial_index>=15 && axial_index<=19) || (axial_index>=28 && axial_index<=32)) )
        {
          SF += (1.-(double)(compt_scatter/compt_emis))*100. ;
          compt_SF++ ;
          //printf("%f %d\n",SF, compt_SF);
        }
    ...
    if (segment_index==0)
      printf("Scatter Fraction for segment 0 : %f\n",(double)(SF/compt_SF));

  } // end of for loop for segment range

  save_and_delete_segments(output, segments_out, sino1_data_ptr->get_min_segment_num(), sino1_data_ptr->get_max_segment_num(), *proj_data_out_ptr);
  proj_data_out_ptr = 0;
