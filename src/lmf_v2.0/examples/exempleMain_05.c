...
int main()
{
  ...
  */
  ...
  if(openFile(pToCcsFile,ccsFileName)) return(EXIT_FAILURE);
  ...
  */ 
  ...
  if((openFile(result,"result.data"))==EXIT_FAILURE) exit(EXIT_FAILURE);  
  
  for(numericalValue[0]=0;numericalValue[0]<(u16)pEncoH->scannerTopology.totalNumberOfRsectors;numericalValue[0]++){
    for(numericalValue[1]=0;numericalValue[1]<(u16)pEncoH->scannerTopology.totalNumberOfModules;numericalValue[1]++){
      for(numericalValue[2]=0;numericalValue[2]<(u16)pEncoH->scannerTopology.totalNumberOfSubmodules;numericalValue[2]++){
	for(numericalValue[3]=0;numericalValue[3]<(u16)pEncoH->scannerTopology.totalNumberOfCrystals;numericalValue[3]++){
	  for(numericalValue[4]=0;numericalValue[4]<(u16)pEncoH->scannerTopology.totalNumberOfLayers;numericalValue[4]++){
	    /* with the rsector ID, the module ID, the submodule ID, the crystal ID and the layer ID,
	       the makeid function find the event ID */
	    pER->crystalIDs[0]=makeid(numericalValue[0],numericalValue[1],numericalValue[2],
	    ...
	    fprintf(result,"%d %d %d %d %d %d %d %f %f %f\n",numericalValue[0],numericalValue[1],numericalValue[2],numericalValue[3],
		    numericalValue[4],numericalValue[5],numericalValue[6],
