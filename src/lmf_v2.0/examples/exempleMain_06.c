 ...
 */
 ...
 if(!pcC)
   {
     printf("*** ERROR : exempleMain_06.c : impossible to malloc for pcC\n");
     exit(0);
   }
  ...
  */ 
  ...
  if ((pEncoHC == NULL)||(pEHC == NULL)||(pGDHC == NULL)||(pCRHC == NULL))
...
/* // 0 is the mode to write this coinci in a file */
  ...
  if(pEncoH->scanContent.eventRecordBool)
    ...
    if(pEncoH->scanContent.gateDigiRecordBool == 1)
    {
      ...
      destroyGDRreader(pER->pGDR); /* // free complete this pGDR in readOneGateDigiRecord.c */
