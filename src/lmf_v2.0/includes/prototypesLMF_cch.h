...
result_unit_conversion findUnitConversionFactor(content_data_unit unit, 
						content_data_unit default_unit,
						int unit_type);
struct tm modifyDateFormat(i8 date[charNum],
			   i8 field[charNum]);
...
struct tm modifyTimeFormat(i8 time[charNum],
			   i8 field[charNum]);
...
int setShiftValues(struct LMF_ccs_encodingHeader *pEncoHforGeometry, int cch_index);
...
int findStartNumber(i8 stringbuf[charNum],struct LMF_ccs_encodingHeader *pEncoHforGeometry, int structure_index, int cch_index);
