...
void init5rings(const ENCODING_HEADER *pEncoH, ENCODING_HEADER **ppEncoHC)
{
  int cch_index=0;
		...
		ENCODING_HEADER **ppEncoHC)
{
  ...
  sct = pcrist[4];
  localSubstructureID = locateID(pcrist,substructureOrder,first_pSubstructuresNumericalValues);
  ...
  if(sct%2)
	...
	*ppER = NULL;
	...
	{
	  axialNb -= 3;
	  pcrist[1] = (int)pSubstructuresNumericalValues[substructureOrder] * axialNb + tangNb;
	  (*ppER)->crystalIDs[0] = makeid(pcrist[4],pcrist[3],pcrist[2],pcrist[1],pcrist[0],pEncoH,&errFlag);
	}
    }
  ...
  if(*ppER)
	...
	if(sct%2)
	...
	else
	  {
		...
		axialNb -= 3;
		pcrist[1] = (int)pSubstructuresNumericalValues[substructureOrder] * axialNb + tangNb;
		(*ppER)->crystalIDs[1] = makeid(pcrist[4],pcrist[3],pcrist[2],pcrist[1],pcrist[0],pEncoH,&errFlag);
	      }
	  }
