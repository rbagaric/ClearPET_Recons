...
/*-------------------------------------------------------
     ...
     TTTT sSSc FrbR RRRR
     and a LMF_ccs_encodingHeader structure pointer
...
i16 calculateSSrepeat(i16 SSpart,const ENCODING_HEADER *pEncoH)
{
 ...
 if(SSpart == 2)
    {
    SSrepeat =((pEncoH->scannerTopology.totalNumberOfModules)*(pEncoH->scannerTopology.totalNumberOfRsectors));
    }
 if(SSpart == 3)
    {
      SSrepeat =((pEncoH->scannerTopology.totalNumberOfRsectors)*(pEncoH->scannerTopology.totalNumberOfModules)*(pEncoH->scannerTopology.totalNumberOfSubmodules));
    }
 return(SSrepeat);
...
/* main() */
/* { */
/*   struct LMF_ccs_encodingHeader encodingHeader; */
