  ...
  if(!setDone)
	...
	mode = 3 combined mode with a part of ratioOfModeParalysable (between 0 and 1)
      */
      ...
      printf("mode = 3 combined mode with a part of ratioOfModeParalysable (between 0 and 1)\n");
...
int getElementID(const ENCODING_HEADER *pEncoH, const EVENT_RECORD *pER)
{
  ...
  if(depthOfDeadTime<=5)
	...
	{
	case 0:
	  value = tnm*(int)pcrist[4] + tns*(int)pcrist[3] + tnc*(int)pcrist[2] + tnl*(int)pcrist[1] + (int)pcrist[0];
	  break;
	case 1:
	  value = tnm*(int)pcrist[4] + tns*(int)pcrist[3] + tnc*(int)pcrist[2] + (int)pcrist[1];
	  break;
	case 2:
    ...
    {
      printf("*** error: deadTimeMgr.c : depth must be < 6 ; here depth = %d\n",depthOfDeadTime);
      exit(0);
    }
  free(pcrist);
		...
		const EVENT_RECORD *pER)
{
  ...
  if(deadTimeMode) /*  have we got to manage dead time : mode 0 = no dead time  */
      ...
      if(currentTime < pDeadTime[elementID])
	{
	  ...
	  if(deadTimeMode == 3)  /*  combined mode */
	    {
	      if(monteCarloEngine() < ratioOfModeParalysable)
		{
		  pDeadTime[elementID] = currentTime + currentDeadTime;
		}
	    }
  ...
  if(verbose)
	...
	printf("not taken in account \n");
      ...
      printf("This element will be alive again at  %llu us : element %d\n\n\n",pDeadTime[elementID],elementID);
