			...
			const GATE_DIGI_RECORD *pGDR)
{
  ...
  if(pGDH->eventIDBool)
    {
      printf("eventID = %ld",pGDR->eventID[0]);
      if(pEH->coincidenceBool)
	printf("\t 2nd : eventID = %ld",pGDR->eventID[1]);
      printf("\n");
    }

  if((pGDH->multipleIDBool)&&(pEH->coincidenceBool))
    printf("multipleID = %ld\n",pGDR->multipleID);
  ...
  if(pGDH->sourceXYZPosBool)
    {
      printf("source pos = %d %d %d",pGDR->sourcePos[0].X,pGDR->sourcePos[0].Y,pGDR->sourcePos[0].Z);
      if(pEH->coincidenceBool)
	printf("\t 2nd :  %d %d %d",pGDR->sourcePos[1].X,pGDR->sourcePos[1].Y,pGDR->sourcePos[1].Z);
      printf("\n");
    }
  if(pGDH->globalXYZPosBool)
    {
      printf("global pos = %d %d %d\n",pGDR->globalPos[0].X,pGDR->globalPos[0].Y,pGDR->globalPos[0].Z);
