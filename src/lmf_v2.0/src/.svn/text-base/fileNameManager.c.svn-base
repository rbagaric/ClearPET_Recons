...
int get_extension_ccs_FileName(i8 extension[charNum]){
  ...
  if((strcmp(extension,"_coinci"))==0){
    ...
    if(get_extension_cch_FileName(extension)!=EXIT_SUCCESS) return(EXIT_FAILURE);
  }
  else if((strcmp(extension,"_bis"))==0){
    initialize(bis_ccsFileName);
    strcpy(bis_ccsFileName,inputFileName);
    
    if(get_extension_cch_FileName("_bis")!=EXIT_SUCCESS) return(EXIT_FAILURE);
  }
  else{
...
int get_extension_cch_FileName(i8 extension[charNum]){
  ...
  if((strcmp(extension,"_coinci"))==0){
    ...
    if(copyFile(cchFileName,coinci_cchFileName)!=EXIT_SUCCESS) return(EXIT_FAILURE);
    initialize(newData);
    strcpy(newData,fileName);
    strcat(newData,extension);
    if(modifyDataInFile("scan file name",newData,coinci_cchFileName)!=EXIT_SUCCESS) return(EXIT_FAILURE);
  }
  else if((strcmp(extension,"_bis"))==0){
    initialize(bis_cchFileName);
    strcpy(bis_cchFileName,inputFileName);
    
    if(copyFile(cchFileName,bis_cchFileName)!=EXIT_SUCCESS) return(EXIT_FAILURE);
    initialize(newData);
    strcpy(newData,fileName);
    strcat(newData,extension);
    if(modifyDataInFile("scan file name",newData,bis_cchFileName)!=EXIT_SUCCESS) return(EXIT_FAILURE);
  }
  else{
    printf(ERROR33,fileName,extension,fileName,fileName);
    return(EXIT_FAILURE);
...
{  
  ...
  if(copyFile(cchFileName,inputFileName)!=EXIT_SUCCESS) 
    return(EXIT_FAILURE);
  if(modifyDataInFile("scan file name",newName,inputFileName)!=EXIT_SUCCESS)
    return(EXIT_FAILURE);
