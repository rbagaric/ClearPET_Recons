...
int fillInStructScannerGeometry(int cch_index,  LMF_cch_scannerGeometry *pScanGeo){
  ...
  for(list_index=0;list_index<21;list_index++){
    for(cch_index=0;cch_index<=last_cch_list;cch_index++){
       if(strcasecmp(generalScannerGeometryInfo[list_index],plist_cch[cch_index].field)==0){
