...
contentLMFdata getLMF_cchNumericalValue(i8 field[charNum]){
  ...
  if(strlen(field)>=charNum) printf(ERROR4,charNum);
  for(cch_index=0;cch_index<=last_cch_list;cch_index++){
    if(strcasecmp(plist_cch[cch_index].field,field)==0) break;
  } 
  ...
  valueToReconstruction.numericalValue=plist_cch[cch_index].def_unit_value.vNum;
