...
void setPileUpAnalysis(u8 bool){pileUpAnalysis = bool;}
void setHighDepthDT(u64 value){PMDT = value;}
void setLowDepthDT(u64 value){FPGADT = value;}
void setThreeSingleDT(u64 value){threeSingleDT = value;}
	       ...
	       const i8 *nameOfFile)
{
  ...
  if(doneOnce == FALSE)
      ...
      if(pEH->detectorIDBool == FALSE)
	{
      ...
      pBufJuelich = (struct bufferJuelich**)malloc(sizeof(struct bufferJuelich)*tnr);
      if(pBufJuelich == NULL)
	printf("*** ERROR : juelichDT.c : malloc\n");
      for(i=0;i<tnr;i++)
	{
	  pBufJuelich[i] = (struct bufferJuelich*)malloc(sizeof(struct bufferJuelich)*tnm);
	 ...
	 for(j=0;j<tnm;j++)
	   {
	     ...
	     if (verbose) printf("allocation of pbuf[%d][%d] ok \n",i,j);
	   }
	}
  ...
  if (verbose) printf("A new event has come = %d %llu %d %d (number,time,pm,fpga)\n",
		      nSinglesTreated++, getTimeOfThisEVENT(pER), getModuleID(pEncoH,pER),getRsectorID(pEncoH,pER));
  
 
  if (verbose)   printf("\t\tdiff = %llu\n",getTimeOfThisEVENT(pER)-diffP);

  if (verbose)   diffP=getTimeOfThisEVENT(pER);
  ...
  if(sectorIsAlive(erSectorID,erTime))
    {
      if (verbose)      printf("sector %d is alive \n",erSectorID);

      if(moduleIsAlive(erSectorID,erModuleID,erTime))
	{
	  if (verbose) printf("module %d is alive \n",erModuleID);
	  
	  for(j=0;j<tnm;j++)	  
	    {
	      if (pBufJuelich[erSectorID][j].unfreeBool == TRUE) // if an event was waitin write it
		{
		  if (verbose)  printf("an event was waiting in pBufJuelich[%d][%d] ...write it\n",erSectorID,j);
		  LMFbuilder(pEncoH,pEH,pCRH,pGDH,pcC,pBufJuelich[erSectorID][j].pERbuf,pCRR,&pf,bis_ccsFileName);
		}
	      else
		{

		  if (verbose)  printf("no event was waiting in pBufJuelich[%d][%d] \n",erSectorID,j);
		}
	  ...
	  if (verbose)  printf("copy this new event in  pBufJuelich[%d][%d] and unfreeBool is tag as true\n",erSectorID,erModuleID);	  
	  ...
	  if(!pileUpAnalysis)
	    pBufJuelich[erSectorID][erModuleID].nextTimePMalive = erTime + PMDT;
	  else
	    pBufJuelich[erSectorID][erModuleID].nextTimePMalive = erTime + FPGADT;
	  // PMDT=400000, FPGADT=250000
	
	  if (verbose)  printf("tune next alive time for sector %d to %llu and module %d to %llu\n",
			       erSectorID,
      ...
      else
	{
	  ...
	  if (verbose) printf("module %d is dead tag pBuf[%d][%d],unfree to FALSE \n",erModuleID,erSectorID,erModuleID);  
...
u8 sectorIsAlive(int erSector,u64 timeER) /* check if one PM of this FPGA is dead */
{
  int iMod=0;
  u8 alive = TRUE;
...
u8 moduleIsAlive(int erSector,int erModule,u64 timeER)/* check if this PM is dead */
{
  u8 alive = TRUE;
