...
calculOfEventPosition locateEventInLaboratory(const ENCODING_HEADER *pEncoH,
					      ...
					      int indexID)
{
  ...
  double eventPosX=0, eventPosY=0, eventPosZ=0, angularRsectorPos=0;
  ...
  if(initializationDone==FALSE){
    ...
    if(testPitchVersusSize(first_pSubstructuresNumericalValues,pScanGeo, first_pRdSizeLayers)==1) exit(EXIT_FAILURE);
    ...
    if(((double)(pEncoH->scannerTopology.numberOfSectors-1)*pScanGeo->rsectorAzimuthalPitch*angleDefaultUnitToRadConversionFactor)>6.283185307){
      printf(ERROR_GEOMETRY10,pScanGeo->rsectorAzimuthalPitch,(int)pEncoH->scannerTopology.numberOfSectors);
      printf(ERROR5,cchFileName);
      exit(EXIT_FAILURE);
    }
  ...
  resultOfCalculOfEventPosition.substructureInRsector3DPosition = locateSubstructureInStructure( pcrist,
												 1,
  ...
  rotationAngle=(double)(((double)rsectorID.tangential*(pScanGeo->rsectorAzimuthalPitch*angleDefaultUnitToRadConversionFactor)));
  angularRsectorPos=(double)((pScanGeo->azimuthalStep*angleDefaultUnitToRadConversionFactor)*(double)pER->gantryAngularPos);
  
  resultOfCalculOfEventPosition.substructureInScanner3DPosition.radial =(-(resultOfCalculOfEventPosition.substructureInRsector3DPosition.tangential 
									   * sin(rotationAngle)));  
  
  resultOfCalculOfEventPosition.substructureInScanner3DPosition.tangential=(resultOfCalculOfEventPosition.substructureInRsector3DPosition.tangential 
									    * cos(rotationAngle)); 
  
  resultOfCalculOfEventPosition.substructureInScanner3DPosition.axial =resultOfCalculOfEventPosition.substructureInRsector3DPosition.axial;
  
  /**** Calculation of the 3D coordinates of rsector (id_t,id_z) in the 3D laboratory (x,y,z) system ****/ 
  resultOfCalculOfEventPosition.rsectorInLaboratory3DPosition = locateRsectorInLaboratory(pcrist, 
											  pScanGeo, 
											  pEncoH, 
											  pER, 
  ...
  eventPosX=resultOfCalculOfEventPosition.rsectorInLaboratory3DPosition.radial
    +resultOfCalculOfEventPosition.substructureInScanner3DPosition.radial
    +ppShiftValuesList[pcrist[4]][0];
  
  eventPosY=resultOfCalculOfEventPosition.rsectorInLaboratory3DPosition.tangential
    +resultOfCalculOfEventPosition.substructureInScanner3DPosition.tangential
    +ppShiftValuesList[pcrist[4]][1];
  
  eventPosZ=resultOfCalculOfEventPosition.rsectorInLaboratory3DPosition.axial
  +resultOfCalculOfEventPosition.substructureInScanner3DPosition.axial
  +ppShiftValuesList[pcrist[4]][2];


  resultOfCalculOfEventPosition.eventInLaboratory3DPosition.radial= (eventPosX*cos(angularRsectorPos))-(eventPosY*sin(angularRsectorPos));
  resultOfCalculOfEventPosition.eventInLaboratory3DPosition.tangential= (eventPosX*sin(angularRsectorPos))+(eventPosY*cos(angularRsectorPos));
  resultOfCalculOfEventPosition.eventInLaboratory3DPosition.axial= eventPosZ;
...
void destroyScannerGeometryPointers(){
  if(initializationDone==TRUE){
    if(first_pIntLengthLayers != NULL) free(first_pIntLengthLayers);
    if(first_pRdSizeLayers != NULL) free(first_pRdSizeLayers);
