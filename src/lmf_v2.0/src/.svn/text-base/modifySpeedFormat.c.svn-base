...
complex_unit_type modifySpeedFormat(i8 undefined_unit[charNum]){
  ...
  while((line=strtok(NULL," "))!=NULL) strcat(undefined_unit,line);
  ...
  if((line=strchr(undefined_unit,'/'))==NULL){
	...
	while((line=strtok(NULL,"\0"))!=NULL) strcpy(complex_unit.denominator,line);
      }
    }
	...
	if(strncmp(end_word,"p",1)==0){
	  ...
	  if(strncmp(complex_unit.denominator,"m",1)==0) strcat(complex_unit.denominator,"in");
	  strncpy(complex_unit.numerator,undefined_unit,(unit_length-denominator_length));
	  break;
	}
	end_word--;
	unitIndex++;
  ...
  else strcpy(complex_unit.numerator,"error");
