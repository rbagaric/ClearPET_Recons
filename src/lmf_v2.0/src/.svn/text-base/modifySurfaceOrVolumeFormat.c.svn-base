...
i8 *modifySurfaceOrVolumeFormat(i8 unit[charNum],int unit_type,i8 power[charNum]){
  ...
  while((line=strtok(NULL," *^x."))!=NULL) strcat(unit,line);
  ...
  if(unit_length>1){
    ...
    end_word--;
    initialize(stringbuf);
    if((strncmp(end_word,power,1))==0) strncpy(stringbuf,unit,(unit_length-1));
  }
  else strcpy(stringbuf,"error");
