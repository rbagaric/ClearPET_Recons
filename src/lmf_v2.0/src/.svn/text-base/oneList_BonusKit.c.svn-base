...
GATE_DIGI_RECORD (*newGDR(void))
{
  ...
  if ((pGDR = (GATE_DIGI_RECORD *)malloc(sizeof(GATE_DIGI_RECORD))) == NULL)
    {
      printf("\n*** ERROR : oneList_bonus.c : impossible to do malloc\n");
      exit(1);
    }
...
********************************************************/
void copyER(const EVENT_RECORD *pER_source,EVENT_RECORD *pER_destination,const EVENT_HEADER *pEH)
{
  int i;
  for (i=0;i<8;i++)
  ...
  for (i = 0 ; i < ((pEH->coincidenceBool+1)*(pEH->numberOfNeighbours+1)) ; i++)
    {
...
********************************************************/
void copyGDR(GATE_DIGI_RECORD *pGDR_source,GATE_DIGI_RECORD *pGDR_destination)
{
	  ...
	  if (dlist_is_tail(element))
	  ...
	  else
	    {
	      ...
	      if ( getTimeOfThisEVENT((EVENT_RECORD *)element->data) <=
		   (getTimeOfThisEVENT((EVENT_RECORD *)element->next->data) + coincidence_window))
		{
		  printf("\t+");
		}
...
LINK searchRecursiveFromHead(const LIST *plist, EVENT_RECORD *pNew)
{
  ...
  if(dlist_head(plist) != NULL)
      ...
      else if(myPrev->next == NULL)
	{
      ...
      if((tNew <= getTimeOfThisEVENT((EVENT_RECORD*)(myPrev->data))) &&
	 (tNew >= getTimeOfThisEVENT((EVENT_RECORD*)(myPrev->next->data))))
	{
	  prevAssigned = FALSE;
	  return(myPrev);	
	}
...
LINK searchIterativeFromTail(const LIST *plist,EVENT_RECORD *pNew)
{
  ...
  while(1)
      ...
      else
	{
	  ...
	  tCurrent = getTimeOfThisEVENT((EVENT_RECORD*)current->data);
	  tCurrentNext = getTimeOfThisEVENT((EVENT_RECORD*)current->next->data);
