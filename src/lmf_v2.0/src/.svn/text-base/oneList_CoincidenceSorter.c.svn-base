...
int getAndSetCSsearchMode()
{
  ...
  if (!searchingModeSet)
    {
      printf("Choose searching mode to locate elements in list (recursive mode recommanded) :\n");
  ...
  if (!coincidenceDeadTimeValueSet)
    {
      ...
      if(cdtm)
	{
	  printf(" Value of dead time for coincidence writting (picoseconds):");   
	  scanf("%llu",&cdtv); getchar();/*   user choice */
	}
      else
	cdtv = 0;
...
int setCSverboseLevel()
{
  ...
  if (!verboseSet)
    {
      printf("Enter verbose level for coincidence sorter : (0 recommanded)\n");
      /*       vL = 0; */
      vL = hardgeti16(0,9);
...
int setCSsaveAutoCoinciBool()
{
  ...
  if (!autoCoinciBoolSet)
    {     
      ...
      /*        ------------------------------ */
      /*        2. ask to user : */
      printf("Do you want to save auto coincidences (same Rsector) ? (no recommanded) :");      
...
int setCSrsectorNeighOrder()
{
  ...
  if (!rsectorNeighOrderSet)
    {     
      ...
      printf("coincidences are accepted if |rsectorID1 - rsectorID2}| > 2 ) ");    
...
int setCSstackcuttime()
{
  ...
  if (!sctSet)
    {
      printf("Enter stack cut time (microseconds) ( 10 to 50 recommanded and 0 for help) : ");
      sct=10;
      ...
      scanf("%llu",&sct); getchar(); /*  user choice */
  ...
  *****************************************/
  ...
  if(headDone == FALSE)/*   only once-done block  */
      ...
      if(verboseLevel)
	{
      ...
      if(verboseLevel) printf("Diamond 1\n");
      /****************************************
      ...
      if(listP1.size > 0)
	{
	  ...
	  if(verboseLevel) printf("Diamond 5\n");
	  /****************************************
	  ...
	  *****************************************/
	  ...
	  if(getTimeOfThisEVENT((EVENT_RECORD*)dlist_head(&listP1)->data) >= (tEn + stack_cut_time) )
	    {
	      ...
	      if(verboseLevel)
		{
		  ...
		  printf("SCT = %llu\n",stack_cut_time);
		  printf("head P1 = %llu\n",getTimeOfThisEVENT((EVENT_RECORD*)dlist_head(&listP1)->data));
