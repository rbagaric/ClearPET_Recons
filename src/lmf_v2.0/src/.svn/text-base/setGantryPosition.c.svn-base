...
{
  angularShift = roundM((float) (inlineAngularShift) * MILLI2PICO * 1000 / getTimeStepFromCCH() / clkRatio);
  axialShift = roundM((float) (inlineAxialShift) * MILLI2PICO * 1000 / getTimeStepFromCCH() / clkRatio);
}
	  ...
	  if(dlist_prev(element))
	    {
	      ...
	      if ((currentEv->counts + angularShift) - evCnts > evCnts - (previousEv->counts + angularShift))
		{
  ...
  while (1)
      ...
      if ((currentEv->counts + axialShift) >= evCnts)
	{
	  if(dlist_prev(element))
	    {
	      previousEv = (gantryEVENT *) (dlist_prev(element)->data);
	      if ((currentEv->counts + axialShift) - evCnts > evCnts - (previousEv->counts + axialShift))
		{
		  currentEv = previousEv;
		  element = dlist_prev(element);
		}
...
void GetQuadraticParams(float * a, float * b, float * c, 
			float x1, float f1, float x2, float f2, float x3, float f3)
{
  *a = (1/((x1 - x2)*(x1 - x3))) * f1 + (1/((-x1 + x2)*(x2 - x3))) * f2 + (1/((x1 - x3)*(x2 - x3))) * f3;
  *b = -((x2 + x3)/((x1 - x2)*(x1 - x3))) * f1 + ((x1 + x3)/((x1 - x2)*(x2 - x3))) * f2 -
    ((x1 + x2)/((x1 - x3)*(x2 - x3))) * f3;
  *c = ((x2*x3)/((x1 - x2)*(x1 - x3))) * f1 + ((x1*x3)/(-(x1*x2) + x2*x2 + x1*x3 - x2*x3)) * f2 +
    ((x1*x2)/((x1 - x3)*(x2 - x3))) * f3;
}
...
void findPositions(u32 evCnts, EVENT_RECORD **ppER)
{
  ...
  if(!doneOnce)
	...
	printf("ERROR GEoME\n");
      ...
      ringCirc = roundPosL(TWO_PI/(pScanGeo->azimuthalStep * angleDefaultUnitToRadConversionFactor));
  ...
  while (1)
      ...
      if (t2 >= evCnts)
	{
	  ...
	  if(dlist_prev(element))
		...
		(*ppER)->gantryAngularPos = roundM(ang1 + (ang2 - ang1 + ringCirc) * (evCnts - t1) / (t2 - t1)) % ringCirc;
	      else
		(*ppER)->gantryAngularPos = roundM(ang1 + (ang2 - ang1) * (evCnts - t1) / (t2 - t1));
	      (*ppER)->gantryAxialPos = roundM(ax1 + (ax2 - ax1) * (evCnts - t1) / (t2 - t1));
	      /* 	      printf ("%lu => %lu %.0f %.0f\n",evCnts, t1, ang1, ax1); */
				...
				EVENT_RECORD *pER)
{
  ...
  if(setMode == 255)
	...
	evCnts = roundL((double)getTimeOfThisCOINCI(pER) / (CONVERT_TIME_COINCI * getTimeStepFromCCH() / 1000 * clkRatio));
      else
	evCnts = roundL((double)getTimeOfThisEVENT(pER) / (getTimeStepFromCCH() / 1000 * clkRatio));
