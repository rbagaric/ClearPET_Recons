...
int setLayersInfo(int cch_index, 
  ...
  for(list_index=0;list_index<rdNbOfLayers;list_index++){
    if((pIntLengthDescriptionList[list_index]=calloc(sizeOfString,sizeof(i8*)))==NULL){
      printf(ERROR_GEOMETRY9);
      return(EXIT_FAILURE);
    }
  
    if((pRdSizeDescriptionList[list_index]=calloc(sizeOfString,sizeof(i8*)))==NULL){
      printf(ERROR_GEOMETRY9);
      exit(EXIT_FAILURE);
    }
  }
  ...
  for(list_index=0;list_index<rdNbOfLayers; list_index++){
    sprintf(pIntLengthDescriptionList[list_index],"in layer%d interaction length",list_index);
    sprintf(pRdSizeDescriptionList[list_index],"layer%d radial size",list_index);
  }
  pIntLengthLayers=first_pIntLengthLayers;
  
  for(list_index=0;list_index<rdNbOfLayers;list_index++){
    for(cch_index=0;cch_index<=last_cch_list;cch_index++){
      if(strcasecmp(pIntLengthDescriptionList[list_index],plist_cch[cch_index].field)==0){
  ...
  for(list_index=0;list_index<rdNbOfLayers;list_index++){
    for(cch_index=0;cch_index<=last_cch_list;cch_index++){ 
      if(strcasecmp(pRdSizeDescriptionList[list_index],plist_cch[cch_index].field)==0){
