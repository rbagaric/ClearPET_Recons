	      ...
	      const i8 *nameOfFile)
{
  ...
  if(doneOnce == FALSE) /*  first call */
	...
	{ 
	  ...
	  if(timeList.size ==1)
	    {
	      if(getTimeOfThisEVENT(pERin) < getTimeOfThisEVENT((EVENT_RECORD*)dlist_head(&timeList)->data))
		{
      ...
      else /*  other calls */
	{
	  ...
	  if ((dlist_remove(&timeList, dlist_tail(&timeList), (void **)&data)) == 0 && 
	      ((&timeList)->destroy != NULL)) 
	    {
  ...
  if(thisOne == TRUE)
    LMFbuilder(pEncoH,pEH,pCRH,pGDH,pcC,pERout,pCRR,&pf,bis_ccsFileName);	 
