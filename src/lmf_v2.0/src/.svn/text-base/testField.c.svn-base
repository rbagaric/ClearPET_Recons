...
int testField(int last_lmf_header,int cch_index){
  ...
  if(last_lmf_header<0){
    while(TESTADD==0){
      initialize(reply);
      printf("Add this field \"%s\" in your data base ? (y/n): ",plist_cch[cch_index].field);
      if(*gets(reply)=='y'){
	strcpy(headerFileName,HEADER_FILE);
	lmf_header_infile=fopen(headerFileName,"a");
	if(openFile(lmf_header_infile,headerFileName)!=0) exit(EXIT_FAILURE);
	if(writeNewFieldInDataBase(lmf_header_infile,plist_cch[cch_index].field)!=0) exit(EXIT_FAILURE);
	fclose(lmf_header_infile);
	if((last_lmf_header=readTheLMFcchDataBase(lmf_header_index))<0) exit(EXIT_FAILURE);
	TESTADD=1;
      }
    }
  ...
  for(lmf_header_index=0;lmf_header_index<=last_lmf_header;lmf_header_index++){
    if(strcasecmp(plist_cch[cch_index].field,plist_lmf_header[lmf_header_index].field)==0) break;
  }
  if(strcasecmp(plist_cch[cch_index].field,plist_lmf_header[lmf_header_index].field)!=0){
    while(TESTADD==0){
