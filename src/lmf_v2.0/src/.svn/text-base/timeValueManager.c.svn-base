  ...
  if(doneOnce == FALSE)
    {
      ...
      timeStepFromCCH = (u64)(plist_cch[cch_index].def_unit_value.vNum * 1000); 
	...
	{
	  printf("***error : timeValueManager.c : time Step is null or less than 1 picosecond\n");
	  exit(0);
	}
    }
...
********************************************************/
u64 getTimeOfThisEVENT(const EVENT_RECORD *pER)
{
  
  if(pER == NULL)
    return(0);
  else
    return((timeStepFromCCH*u8ToU64(pER->timeStamp))/1000);
  
}


u64 getTimeOfThisCOINCI(const EVENT_RECORD *pER)
{
  u64 timeInMillis;
  if(pER == NULL)
    return(0);
...
u64 getTimeOfFlightOfThisCOINCI(const EVENT_RECORD *pER)
{
  if(pER == NULL)
    return(0);
  else
    return((timeStepFromCCH*pER->timeOfFlight)/1000);
}
