...
u16 tripletAnalysis(struct LMF_ccs_encodingHeader *pEncoH,
		    EVENT_HEADER *pEH,
		    EVENT_RECORD **ppER)
{
  ...
  fprintf(pfAscii,"%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%llu\t%llu\t%llu\n",
	  ...
	  pER1->energy[0]*GATE_LMF_ENERGY_STEP_KEV,
	  pER2->energy[0]*GATE_LMF_ENERGY_STEP_KEV,
	  pER3->energy[0]*GATE_LMF_ENERGY_STEP_KEV,
	  getTimeOfThisEVENT(pER1),
	  getTimeOfThisEVENT(pER2),
	  getTimeOfThisEVENT(pER3));
