...
int LMFcchReader(i8 inputFile[charNum]){
  ...
  static struct LMF_ccs_encodingHeader *pEncoHforGeometry;
  FILE *pToCcsFile=NULL;

  last_lmf_header=readTheLMFcchDataBase();
  
  /** set the input file name **/
  if(setFileName(inputFile)!=EXIT_SUCCESS) return(EXIT_FAILURE);
  if((last_cch_list=fillInFieldAndData())==0) return(EXIT_FAILURE);
  ...
  if((ppShiftValuesList=calloc((int)pEncoHforGeometry->scannerTopology.totalNumberOfRsectors,sizeof(double)))==NULL)
    {
      printf(ERROR36);
      return(EXIT_FAILURE);
    }
  ...
  for(cch_index=0;cch_index<=last_cch_list;cch_index++)
    {
      if((last_lmf_header=testField(last_lmf_header, cch_index))<0) return(EXIT_FAILURE);
      if(fillInCchDefaultUnitValue(last_lmf_header,cch_index,pEncoHforGeometry)!=0) return(EXIT_FAILURE);
    }
  ...
  if((strcasecmp(fileName,plist_cch[cch_index].def_unit_value.vChar))!=0)
    {
...
void LMFcchReaderDestructor()
{
  ...
  if(plist_lmf_header != NULL) free(plist_lmf_header);
  if(plist_cch != NULL) free(plist_cch);
  plist_lmf_header=NULL;
  plist_cch=NULL;
  for(row_index=0;row_index<(lastShiftValuesList_index+1);row_index++) 
    {
