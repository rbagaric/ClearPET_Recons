			 ...
			 const EVENT_RECORD *pER)
{
  ...
  if(doneOnce==FALSE)
	...
	{
	  printf("*** error : coincidenceAnalyser.c : not an event record file\n");
	  fileOK = FALSE;
	...
	{
	  printf("*** error : coincidenceAnalyser.c : not a coincidence file\n");
	  fileOK = FALSE; 
	  printf("<ENTER> to continue\n");getchar();
	}
      if(pEncoH->scanContent.gateDigiRecordBool == FALSE)
	{
	  printf("*** warning : coincidenceAnalyser.c : no gate digi in this file\n");
	  fileOK = FALSE; 
	  printf("<ENTER> to continue\n");getchar();
	}
      if(pGDH->comptonBool == FALSE)
	{
	  printf("*** warning : coincidenceAnalyser.c : no number of compton in this file\n");
	  fileOK = FALSE; 
	  printf("<ENTER> to continue\n");getchar();
	}
	...
	{
	  printf("*** warning : coincidenceAnalyser.c : no event ID  in this file\n");
	  fileOK = FALSE; 
	  printf("<ENTER> to continue\n");getchar();
	}
      doneOnce = TRUE;
  ...
  if(fileOK)
      ...
      else if(pER->pGDR->numberCompton[0] || pER->pGDR->numberCompton[1])
	{
	  nScatterCoincidences++;
	      ...
	      (pER->pGDR->numberDetectorCompton[0] || pER->pGDR->numberDetectorCompton[1]))
	{  
	      nScatterCoincidences++;
...
void destroyCoincidenceAnalyser()
{
  ...
  printf("trues = %llu\t %f %%\n",nTrueCoincidences,100*((float)nTrueCoincidences)/((float)nTotalCoincidences));
  printf("scatter = %llu\t %f %% : phantom scat = %llu detector scat. = %llu\n",
	 nScatterCoincidences,
	 100*((float)(nScatterDetectorCoincidences))/((float)nTotalCoincidences),
	 nScatterCoincidences-nScatterDetectorCoincidences,
	 nScatterDetectorCoincidences);
  printf("random = %llu\t %f %%\n",nRandomCoincidences,100*((float)nRandomCoincidences)/((float)nTotalCoincidences));  
 

  NEC = ((float)nTrueCoincidences*(float)nTrueCoincidences); // T*T
  NEC = NEC / ( (float)nTrueCoincidences + (float)nScatterCoincidences + 2*factorK*(float)nRandomCoincidences);

  //  printf("NEC (time = 1s and k = %f)  = %f --> \n\n", factorK,NEC);
  

  printf("multiples = %llu\t %f %%\n",nMultiples,100*((float)nMultiples)/((float)nTotalCoincidences));
