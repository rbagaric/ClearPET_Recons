...
static i8 coincidenceOutputMode;
/*
  mode 0 LMF file
...
void outputCoincidence(struct LMF_ccs_encodingHeader *pEncoHC,
		       struct LMF_ccs_eventHeader *pEHC,
		       struct LMF_ccs_gateDigiHeader *pGDHC,
		       struct LMF_ccs_currentContent *pcCC,   
		       struct LMF_ccs_eventRecord *pERC,
		       i8 coincidenceOutputMode)
{
  
  if(coincidenceOutputMode == 0)
    {
      pfC = LMFCbuilder(pEncoHC,pEHC,pGDHC,pcCC,pERC,pfC,coinci_ccsFileName);   /* ...write it */
  ...
  if(coincidenceOutputMode == 1)
	...
	{
	  printf("*** WARNING : coincidenceOutputModule.c : impossible to insert a new event in list\n");
	  printf("exit\n");
	  exit(0);
	}
