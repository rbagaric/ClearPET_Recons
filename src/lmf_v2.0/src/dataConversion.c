...
content_data_unit defineUnitAndValue(i8 data[charNum]){
  ...
  int lengthIndex=0,stringbuf_length=0,unit_length=0,data_length=0;
  i8 *line=NULL;
  i8 *end_word=NULL;
  ...
  if(data_length!=0){
    ...
    while((line=strtok(NULL," "))!=NULL) strcat(stringbuf,line);
    stringbuf_length=strlen(stringbuf);
    
    if(((isdigit(stringbuf[0]))!=0)||((strncasecmp(stringbuf,"+",1))==0)||((strncasecmp(stringbuf,"-",1))==0)){
      end_word=strchr(stringbuf,'\0');
      end_word=end_word-2;

      while(lengthIndex<stringbuf_length){
	if((isdigit(end_word[0])!=0)||((strncasecmp(end_word,".",1))==0)){
	  end_word++;
    ...
    else strcpy(content_data.unit,stringbuf);
  }
...
content_data_unit definePrefixAndUnit(i8 undefined_unit[charNum], int unit_type){
  ...
  for(unitIndex=0;unitIndex<nb_units[unit_type];unitIndex++){
    ...
    if((strncasecmp(comp_unit,symbol_units_list[dataType][unitIndex],reply_length))==0){
...
double findPrefixConversionFactor(content_data_unit unit, 
  ...
  for(prefixIndex=0;prefixIndex<16;prefixIndex++){
    if((strcmp(unit.prefix,standard_prefix_list[prefixIndex]))==0) break;
  }
  ...
  for(defaultPrefixIndex=0;defaultPrefixIndex<16;defaultPrefixIndex++){
    if((strcmp(default_unit.prefix,standard_prefix_list[defaultPrefixIndex]))==0) break;
  }
  
  if((strcmp(default_unit.prefix,standard_prefix_list[defaultPrefixIndex]))!=0){
    result=-2;
    return(result);
  }
...
result_unit_conversion findUnitConversionFactor( content_data_unit unit, 
  ...
  for(unitIndex=0;unitIndex<nb_units[dataType];unitIndex++){
    if((strcmp(unit.unit,symbol_units_list[dataType][unitIndex]))==0) break;
  }
  
  for(defaultUnitIndex=0;defaultUnitIndex<nb_units[dataType];defaultUnitIndex++){
    if((strcmp(default_unit.unit,symbol_units_list[dataType][defaultUnitIndex]))==0) break;
  }
  
  result_conversion.factor=units_conversion_factors_list[dataType][unitIndex][defaultUnitIndex];
  result_conversion.constant=units_conversion_constants_list[dataType][unitIndex][defaultUnitIndex];
