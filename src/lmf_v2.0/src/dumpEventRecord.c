		     ...
		     const EVENT_RECORD *pER)
{
  ...
  if(pEH->energyBool == TRUE)
    {
      printf("\n");
      printf("Energy in main crystal = %d --> %d keV\n",pER->energy[0],pER->energy[0]*GATE_LMF_ENERGY_STEP_KEV);
      
      if(pEH->neighbourBool == TRUE)
	{
	  for(j=1; j <= pEH->numberOfNeighbours; j++)
	    printf("Energy in neighbour %d of main crystal = %d --> %d keV\n",j,pER->energy[j],pER->energy[j]*GATE_LMF_ENERGY_STEP_KEV);
	}
      
      if(pEH->coincidenceBool == TRUE)
	{
	  printf("\n");
	  printf("Energy in second crystal = %d --> %d keV\n",pER->energy[pEH->numberOfNeighbours+1],pER->energy[pEH->numberOfNeighbours+1]*GATE_LMF_ENERGY_STEP_KEV);
      	  if(pEH->neighbourBool == TRUE)
	    {
	      for(j=pEH->numberOfNeighbours+2; j <= (2*pEH->numberOfNeighbours)+1; j++)
		printf("Energy in neighbour %d of second crystal = %d --> %d keV\n",j,pER->energy[j],pER->energy[j]*GATE_LMF_ENERGY_STEP_KEV);
	    }
	}
