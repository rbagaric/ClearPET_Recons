...
int fillInFieldAndData(){
  ...
  i8 indata[charNum],stringbuf[charNum],buffer[charNum],copyLine[charNum];
  i8 *fileLine=NULL;
  int indataLength=0,cch_index=0;
  FILE *scanFile;

  scanFile=fopen(cchFileName,"r");
  if(openFile(scanFile,cchFileName)) exit(EXIT_FAILURE);
  ...
  while(fgets(indata,charNum,scanFile)!=NULL){
    ...
    if((plist_cch=allocOfMemoryForLMF_cch(cch_index))==NULL) break;
	...
	strcat(plist_cch[cch_index].data,buffer);
      ...
      else strcpy(plist_cch[cch_index].data,buffer);
    }
