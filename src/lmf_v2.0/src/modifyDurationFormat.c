...
content_data_unit modifyDurationFormat(i8 duration[charNum],i8 field[charNum]){

  i8 *line=NULL,*end_word=NULL;
  i8 buffer[charNum],stringbuf[charNum],duration_info[charNum];
  double duration_list[3]={0};
  int unitIndex=0,formatIndex=0,data_length=0, stringbuf_length=0,buffer_length=0,lengthIndex=0,unit_length=0 ;
  ...
  if((line=strpbrk(stringbuf,"hmHM"))!=NULL){ 
	...
	if((line=strchr(stringbuf,delimiters[unitIndex][formatIndex]))!=NULL){
	  initialize(buffer);
    ...
    if((duration_list[0]>=0 && duration_list[0]<60)!=0) content_data.value=duration_list[0];
    else{
      printf(ERROR49,field,duration);
      printf(ERROR5,cchFileName);
      exit(EXIT_FAILURE);
    }
    if((duration_list[1]>=0 && duration_list[1]<60)!=0) content_data.value+=(duration_list[1]*60);
    else{
      printf(ERROR50,field,duration);
      printf(ERROR5,cchFileName);
      exit(EXIT_FAILURE);
    } 
    if((duration_list[2]>=0 && duration_list[2]<24)!=0) content_data.value+=(duration_list[2]*3600);
    else{
