...
struct tm modifyTimeFormat(i8 time[charNum],i8 field[charNum]){
  ...
  switch(unitIndex){
  case 0:
    if((time_list[0]>=0 && time_list[0]<60)!=0) result_time.tm_sec=time_list[0];
    else{
  ...
  case 1:
    ...
    if((time_list[1]>=0 && time_list[1]<60)!=0) result_time.tm_min=time_list[1];
    else{
  ...
  case 2:
    ...
    if((time_list[2]>=0 && time_list[2]<24)!=0) result_time.tm_hour=time_list[2];
    else{
