...
int writeNewFieldInDataBase(FILE *lmf_header_infile, i8 field[charNum]){
  ...
  i8 field_type_num[20][2]={"1","2","3","4","5","13","19","6","7","8","9","10",
...
int defineFieldType(i8 field[charNum]){
  ...
  printf("To add the field \"%s\" in your data base, you must select the type of its value.\n",field);
  ...
  while(TESTDEF==0){
    ...
    for(fieldType=0;fieldType<20;fieldType++){
      ...
      while(TESTANSWER==0){
	printf("Is the value of the field \"%s\" %s (y/n): ",field,information_type[fieldType]);
