		 ...
		 const EVENT_RECORD *pER)
{
  if(doneOnce==FALSE)
    {      
      if(pEncoH == NULL)
	printf("*** warning : outputAsciiMgr.c : no encoding pointer pEncoH defined, please check \n");
      else
	{
	  if(pEncoH->scanContent.eventRecordBool == FALSE)
	    {
	      printf("*** error : outputAsciiMgr.c : not an event record file\n");
	      //fileOK = FALSE;
	  ...
	  if(pEncoH->scanContent.gateDigiRecordBool == FALSE)
	    {
	      printf("*** warning : outputAsciiMgr.c : no gate digi in this file\n");
	      //fileOK = FALSE; 
	      printf("<ENTER> to continue\n");getchar();
	    }
	}
      ...
      if(pGDH == NULL)
	printf("*** warning : outputAsciiMgr.c : no encoding pointer pGDH defined, please check \n");
      else 
	{	
	  if(pGDH->comptonBool == FALSE)
	    {
	      printf("*** warning : outputAsciiMgr.c : no number of compton in this file\n");
	      //fileOK = FALSE; 
	      printf("<ENTER> to continue\n");getchar();
	    }
	  if(pGDH->eventIDBool == FALSE)
	    {
	      printf("*** warning : outputAsciiMgr.c : no event ID  in this file\n");
	      //fileOK = FALSE; 
	      printf("<ENTER> to continue\n");getchar();
	    }
	}
  ...
  if(fileOK)
      ...
      if(pEH->coincidenceBool)
	{
	  ...
	  fprintf(pfile,"%llu \t%llu \t%llx \t%llx",
		  timeMillis,
		  getTimeOfFlightOfThisCOINCI(pER),
		  pER->crystalIDs[0],
		  pER->crystalIDs[1]); 
	  if( pGDR != NULL)fprintf(pfile,"\t%ld \t%ld",pER->pGDR->eventID[0], pER->pGDR->eventID[1]);
	  ...
	  /*     // display XYZ pos for an event record, adapted from processRecordCarrier*/
	    ...
	    if(pEH->energyBool == TRUE)
	      {
		fprintf(pfile,"E %d %d keV",pER->energy[0],pER->energy[0]*GATE_LMF_ENERGY_STEP_KEV);	  
		if(pEH->neighbourBool == TRUE)
		  {
		    for(j=1; j <= pEH->numberOfNeighbours; j++)
		      fprintf(pfile,"Engbr %d %d %d keV", j,pER->energy[j],pER->energy[j]*GATE_LMF_ENERGY_STEP_KEV);
		  }
	      }
	    fprintf(pfile,"\n");
	}
      ...
      else
	{
	  /*   // time_in_picos    crystalID    eventID */
       	  fprintf(pfile,"Ti: %llu \t%llx ",getTimeOfThisEVENT(pER),pER->crystalIDs[0]);
	  if(pGDR != NULL)fprintf(pfile,"\t%ld",pER->pGDR->eventID[0]);
	  ...
	  if(pEH->energyBool == TRUE)
	    {
	      fprintf(pfile," Ey: \t%d  \t%d keV",pER->energy[0],pER->energy[0]*GATE_LMF_ENERGY_STEP_KEV);	  
	      if(pEH->neighbourBool == TRUE)
		{
		  for(j=1; j <= pEH->numberOfNeighbours; j++)
		    fprintf(pfile," \tEngbr % d %d %d keV", j,pER->energy[j],pER->energy[j]*GATE_LMF_ENERGY_STEP_KEV);
