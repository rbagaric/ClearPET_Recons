...
static int allocER = 0;

EVENT_RECORD *readOneEventRecord(u8 *pBufEvent,
				 u16 Epattern,
				 EVENT_HEADER *pEH,
				 u16 encodingIDSize)
{
  ...
  if (!allocER)
    {                   /* just one time this block*/
      if((pER = malloc(sizeof(EVENT_RECORD)))==NULL)
	printf("\n***ERROR : in readOneEventRecord.c : impossible to do : malloc()\n");
  ...
  if ((Epattern & BIT12) == 0) /* if c = 0 */
    { /* Time stamp on 64 bits */
      if ((*punch & BIT16) == 0) /* if tag ok */
	{
	  for(i=0;i<8;i++)
	    { /* read the 8 bytes of time */
  ...
  if(((Epattern&BIT11)==BIT11)&&((Epattern&BIT12)==BIT12))/* if d=1 and c=1 extraction of ID2 */
      ...
      for (size = 0; size < encodingIDSize + 1; size++)
	{
	  *punch = ntohs(*punch);
	  crystalIDs = *punch;
	  punch++;
	  pER->crystalIDs[pEH->numberOfNeighbours+1] |= crystalIDs << (size * 16);
	}
    }
