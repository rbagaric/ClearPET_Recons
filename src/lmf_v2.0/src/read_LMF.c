...
static struct LMF_ccs_currentContent *pcC = NULL;
static struct LMF_ccs_eventRecord *pER = NULL;
static struct LMF_ccs_countRateRecord *pCRR = NULL;

static int doneOnce = FALSE;
static u8 *pBufEvent,*pBufCountRate,*pBufGateDigi;       /* buffer for read*/
static u16 Epattern=0,CRpattern=0,GDpattern=0,pattern=0,sizeE=0,sizeCR=0, sizeGD=0,testread=0;
static u8 uni8=0;
  ...
  if((testread=(fread(&uni8,sizeof(u8),1,pfCCS)))==1)   /* read 1 byte */
      ...
      if((uni8&BIT8)==0)                               /* TAG = 0 ; it s an event */  
	{
	  ...
	  if(testread == sizeE)       /* read the event record in once and fill the */
	    {
	      pER = readOneEventRecord(&pBufEvent[0],Epattern,pEH,pEncoH->scanEncodingIDLength); /* carrier */
	    }
	  ...
	  if(pEH->gateDigiBool) /* extended event record for Gate Digi information */
		...
		{
		  pER->pGDR = readOneGateDigiRecord(&pBufEvent[0],GDpattern,pGDH,pEH); /* carrier */
		}
	      else                                    /* is there a problem of reading ?*/
	...
	{  /*        This record is a count rate ... no treated here ... */
	  if(verboseLevel > 5)
	    printf("a count rate record have been read read but not treated\n");
	 
	}
      else if(pcC->typeOfCarrier == pEncoH->scanContent.eventRecordTag)
	{  /*        This record is an event ... treated here ... */
	  
	  resultOfCalculOfEventPosition = locateEventInLaboratory(pEncoH,pER,0);
	 
	 

	 *x1 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.radial;
	 *y1 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.tangential;
	 *z1 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.axial;

	 returnValue = 1;

	 if(pEH->coincidenceBool == 1)
	   {
	     resultOfCalculOfEventPosition = locateEventInLaboratory(pEncoH,pER,1);
	     

	     *x2 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.radial;
	     *y2 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.tangential;
	     *z2 = resultOfCalculOfEventPosition.eventInLaboratory3DPosition.axial;
	     returnValue = 2;
