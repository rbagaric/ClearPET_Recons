...
/*-------------------------------------------------------
     ...
     Description of surfaceConversion.c:
	...
	Fill in the members value and default_value of the LMF_cch structure with a surface 
...
int surfaceConversion(int cch_index,int dataType ){
  ...
  i8 default_unit_file[charNum],surface_volume_unit[charNum],surface_volume_default_unit[charNum];
  ...
  strcpy(surface_volume_unit,modifySurfaceOrVolumeFormat(plist_cch[cch_index].unit,dataType,"2"));
  
  if(strcmp(surface_volume_unit,"error")==0){
    printf(ERROR13,plist_cch[cch_index].field,plist_cch[cch_index].unit);
    printf(ERROR5,cchFileName);
    return(EXIT_FAILURE);
  }
  strcpy(surface_volume_default_unit,modifySurfaceOrVolumeFormat(plist_cch[cch_index].def_unit,dataType,"2"));
  if(strcmp(surface_volume_default_unit,"error")==0){
    printf(ERROR13,plist_cch[cch_index].field,plist_cch[cch_index].def_unit);
  ...
  if(unit.unit[0]=='\0'){
    printf(ERROR13,plist_cch[cch_index].field,plist_cch[cch_index].data);
    printf(ERROR5,cchFileName);
    return(EXIT_FAILURE);
  }
  default_unit=definePrefixAndUnit(surface_volume_default_unit,dataType);
  ...
  plist_cch[cch_index].def_unit_value.vNum = plist_cch[cch_index].value.vNum 
                                             *(result_prefix_conversion*result_conversion.factor
					       *result_prefix_conversion*result_conversion.factor)
