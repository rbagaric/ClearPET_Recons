...
lmf_header*  allocOfMemoryForLMF_Header(int lmf_header_index){
  ...
  if ((plist_lmf_header=(lmf_header*)realloc(plist_lmf_header,
					     (lmf_header_index+1)*sizeof (lmf_header)))== NULL){
...
LMF_cch* allocOfMemoryForLMF_cch(int cch_index){
  ...
  if ((plist_cch=(LMF_cch*)realloc(plist_cch,(cch_index+1)*sizeof(LMF_cch)))== NULL){
...
void initialize(i8 buffer[charNum]){
  int l;
  for(l=0;l<charNum;l++) buffer[l]=0;
}
...
int copyFile(i8 infileName[charNum], i8 copyingFileName[charNum]){
  ...
  if(openFile(infile,infileName)) return(EXIT_FAILURE);

  outfile=fopen(copyingFileName,"w+");
  if(openFile(outfile,copyingFileName)) return(EXIT_FAILURE);
...
int modifyDataInFile(i8 dataDescription[charNum], i8 newData[charNum], i8 file[charNum]){

  i8 stringbuf[charNum],buffer[charNum],copyLine[charNum],saveInfoFile[10000];
  i8 *fileLine=NULL;
  int dataDescriptionLength=0,lineLength=0,dataIndex=0,TESTFIND=FALSE;
  FILE *scanFile;
  i32 dataPosition=0;

  scanFile=fopen(file,"r+");
  if(openFile(scanFile,file)) return(EXIT_FAILURE);
  ...
  while(fgets(stringbuf,charNum,scanFile)!=NULL){
    ...
    if((strncasecmp(buffer,dataDescription,dataDescriptionLength))==0){
      for(dataIndex=0;dataIndex<10000;dataIndex++) saveInfoFile[dataIndex]=0;
